...
## 3️⃣ POTHOLE DETECTION MODEL (ACTUAL IMPLEMENTATION)

### Model Architecture - YOLOv8 Object Detection
...
INPUT LAYER
└── Shape: (None, 640, 640, 3) - Any size accepted, auto-resized
    │
    ▼
┌─────────────────────────────────────────────────────────────────┐
│ BACKBONE: CSPDarknet53                                          │
│ ├── Conv2D Layers with C2f blocks                               │
...
│ ├── Classification branch (pothole class)                       │
...
│ ├── Class: "pothole"                                            │
│ ├── Confidence: 0.0 - 1.0                                       │
│ └── Multiple detections per image supported                     │
└─────────────────────────────────────────────────────────────────┘
```

### Actual Implementation (FastAPI + YOLO)
```python
from ultralytics import YOLO
from fastapi import FastAPI, UploadFile

# Load trained model
MODEL_PATH = "runs/detect/pothole_v13/weights/best.pt"
model = YOLO(MODEL_PATH)

@app.post("/analyze-image")
async def analyze_image(image: UploadFile, conf_threshold: float = 0.25):
    # Run inference with configurable thresholds
    results = model.predict(image_path, conf=conf_threshold, iou=0.70)
    
    detections = []
    for result in results:
        for box in result.boxes:
            detections.append({
                "class": model.names[int(box.cls[0])],  # "pothole"
                "confidence": float(box.conf[0]),       # 0.94
                "bbox": box.xywh[0].tolist(),           # [x, y, w, h]
                "area_ratio": (w * h) / (img_w * img_h) # For severity calc
            })
    
    # Severity based on total detection area
    severity = "high" if total_area > 0.05 else "medium" if total_area > 0.01 else "low"
